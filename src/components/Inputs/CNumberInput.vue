...
function onChange() {
  ...
  if (numberInput.value?.validity.rangeUnderflow) {
    validationError.value = props.max === undefined ?
        `Value must be larger than ${props.min}` : `Value must be between ${props.min} and ${props.max}`;
    return;
  }

  if (numberInput.value?.validity.rangeOverflow) {
    validationError.value = props.min === undefined ?
        `Value must be smaller than ${props.max}` : `Value must be between ${props.min} and ${props.max}`;
    return;
  }
